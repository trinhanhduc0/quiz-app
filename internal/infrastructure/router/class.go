...
func NewRouterClass(s service.ClassUseCase, r service.RedisUseCase) routerClass {
	return routerClass{
		classUseCase: s,
		redisUseCase: r,
	}
...
func (rc routerClass) joinClass(w http.ResponseWriter, req *http.Request) {
	...
	pkg.SendResponse(w, http.StatusOK, fmt.Sprintf("Joined class: %s", class.ID))
}
...
func (rc routerClass) GetClassRouter(r *Router) {
	r.Router.Handle("/class", service.AuthMiddleware(http.HandlerFunc(rc.getAllClass))).Methods("GET")
	r.Router.Handle("/class", service.AuthMiddleware(http.HandlerFunc(rc.createClass))).Methods("POST")
	r.Router.Handle("/class", service.AuthMiddleware(http.HandlerFunc(rc.updateClass))).Methods("PATCH")
	r.Router.Handle("/class", service.AuthMiddleware(http.HandlerFunc(rc.deleteClass))).Methods("DELETE")
	r.Router.Handle("/getclass", service.AuthMiddleware(http.HandlerFunc(rc.getAllClassByEmail))).Methods("GET")

	r.Router.Handle("/class/codeclass", service.AuthMiddleware(http.HandlerFunc(rc.createCodeClass))).Methods("POST")
	r.Router.Handle("/class/joinclass", service.AuthMiddleware(http.HandlerFunc(rc.joinClass))).Methods("POST")
}
