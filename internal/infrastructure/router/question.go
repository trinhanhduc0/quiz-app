...
func NewRouterQuestion(questionUseCase service.QuestionUseCase) *RoutesQuestion {
...
func (rq *RoutesQuestion) GetQuestionRouter(r *Router) {
	r.Handle("/questions", service.AuthMiddleware(http.HandlerFunc(rq.createQuestions))).Methods("POST")
	r.Handle("/questions", service.AuthMiddleware(http.HandlerFunc(rq.getAllQuestions))).Methods("GET")
	r.Handle("/questions", service.AuthMiddleware(http.HandlerFunc(rq.updateQuestion))).Methods("PATCH")
	r.Handle("/questions", service.AuthMiddleware(http.HandlerFunc(rq.deleteQuestion))).Methods("DELETE")
}
...
func (rq *RoutesQuestion) getAllQuestions(w http.ResponseWriter, req *http.Request) {
	...
	questions, err := rq.questionUseCase.GetAllQuestionsByUser(context.TODO(), userID)
	if err != nil {
...
func (rq *RoutesQuestion) deleteQuestion(w http.ResponseWriter, req *http.Request) {
	...
	pkg.SendResponse(w, http.StatusInternalServerError, question.ID)
}
