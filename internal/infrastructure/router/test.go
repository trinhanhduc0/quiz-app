...
func NewRouterTest(testUseCase service.TestUseCase, classUseCase service.ClassUseCase, questionUseCase service.QuestionUseCase, answerUseCase service.AnswerUseCase, redisUseCase service.RedisUseCase) RoutesTest {
	return RoutesTest{
...
func (rt RoutesTest) GetTestRouter(r *Router) {
	...
	r.Handle("/tests", service.AuthMiddleware(http.HandlerFunc(rt.getAllTestFromAuthor))).Methods("GET")
	r.Handle("/tests", service.AuthMiddleware(http.HandlerFunc(rt.createTest))).Methods("POST")
	r.Handle("/tests", service.AuthMiddleware(http.HandlerFunc(rt.updateTest))).Methods("PATCH")
	r.Handle("/tests", service.AuthMiddleware(http.HandlerFunc(rt.deleteTest))).Methods("DELETE")

	// Routes for class-specific operations
	r.Handle("/tests/class", service.AuthMiddleware(http.HandlerFunc(rt.getAllTestOfClassByEmail))).Methods("POST")

	// Routes for managing questions within tests
	r.Handle("/tests/questions", service.AuthMiddleware(http.HandlerFunc(rt.getQuestionOfTest))).Methods("POST")

	// Routes for marking test completion and sending test results
	r.Handle("/test/done", service.AuthMiddleware(http.HandlerFunc(rt.getDoneTest))).Methods("POST")
	r.Handle("/test/send", service.AuthMiddleware(http.HandlerFunc(rt.sendTest))).Methods("POST")
}
