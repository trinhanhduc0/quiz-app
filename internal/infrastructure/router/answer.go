...
func NewRouterAnswer(s *service.AnswerUseCase, t *service.TestUseCase, q *service.QuestionUseCase, r *service.RedisUseCase) RouterAnswer {
	return RouterAnswer{
		answerUseCase:   s,
		testUseCase:     t,
		redisUseCase:    r,
...
func (rc RouterAnswer) GetAnswerRouter(r *Router) {
	r.Router.Handle("/answer/create", service.AuthMiddleware(http.HandlerFunc(rc.createAnswer))).Methods("POST")
	r.Router.Handle("/answer/get", service.AuthMiddleware(http.HandlerFunc(rc.getAnswer))).Methods("POST")
	r.Router.Handle("/answer/user", service.AuthMiddleware(http.HandlerFunc(rc.getAllAnswerByEmail))).Methods("GET")
}

func (rc RouterAnswer) createAnswer(w http.ResponseWriter, req *http.Request) {
	...
	fmt.Println(newAnswer.TotalScore)

	//fmt.Println(newAnswer)
	err = rc.answerUseCase.CreateNewAnswer(req.Context(), &newAnswer)
	if err != nil {
		pkg.SendError(w, "Failed to create answer: "+err.Error(), http.StatusInternalServerError)
		return
	}
	//rc.testUseCase.RemoveAllowUser(req.Context(), newAnswer.TestId, email)
