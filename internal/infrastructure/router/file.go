...
func NewRoutesFile(usecase *service.FileUseCase, awsS3UseCase *aws.FileAWSRepository) *RoutesFile {
	return &RoutesFile{
		fileUseCase:  usecase,
...
func (rf *RoutesFile) GetRoutesFile(r *Router) {
	r.Router.Handle("/getallimagefile", service.AuthMiddleware(http.HandlerFunc(rf.getAllImageFile))).Methods("GET")
	r.Router.Handle("/getimagefile", service.AuthMiddleware(http.HandlerFunc(rf.getImageFile))).Methods("GET")
	r.Router.Handle("/upimagefile", service.AuthMiddleware(http.HandlerFunc(rf.uploadImageFile))).Methods("POST")

	r.Router.Handle("/upfile", service.AuthMiddleware(http.HandlerFunc(rf.uploadFile))).Methods("POST")
	r.Router.Handle("/getfile", service.AuthMiddleware(http.HandlerFunc(rf.userGetFile))).Methods("POST")
	r.Router.Handle("/file", service.AuthMiddleware(http.HandlerFunc(rf.deleteFile))).Methods("DELETE")
}
...
func (rf *RoutesFile) deleteFile(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		pkg.SendError(w, "Error delete files", http.StatusInternalServerError)
		return
	}
