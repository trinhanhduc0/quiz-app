...
// GetAllQuestions implements repository.QuestionRepository.GetAllQuestions
func (r *QuestionMongoRepository) GetAllQuestionsByUser(ctx context.Context, userID string) ([]any, error) {
	filter := bson.M{"metadata.author": userID}         // Assuming questions are filtered by userID
	allQuestions, err := r.CollRepo.GetAll(ctx, filter) // Fetch all matching questions
	if err != nil {
